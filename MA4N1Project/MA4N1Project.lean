...
theorem trivial_case (M : ℕ) {f : ℤ[X]} (hp : coeff f 0 = 0) : ∃ p n, Nat.Prime p ∧ M ≤ p ∧ (p : ℤ) ∣ f.eval n :=
  let p := minFac (M ! + 1)
  let n := p
...
theorem non_trivial_case {f : ℤ[X]} (hf : f.natDegree ≠ 0) (M : ℕ) (hp : coeff f 0 ≠ 0): ∃ p n, Nat.Prime p ∧ M ≤ p ∧ (p : ℤ) ∣ f.eval n := by
  sorry
  done

-- Let p be a prime and f (x) ∈ Z[X] be non-constant. Then f (x) ≡ 0 mod p is solvable for infinitely many p
open scoped Polynomial in
lemma two.one {f : ℤ[X]} (hf : f.natDegree ≠ 0) (M : ℕ) : ∃ p n, Nat.Prime p ∧ M ≤ p ∧ (p : ℤ) ∣ f.eval n := by
