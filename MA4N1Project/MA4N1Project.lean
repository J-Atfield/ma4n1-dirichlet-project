...
-- theorem my_g_constant_term (f : ℤ[X]) (hf : f.natDegree ≠ 0) (h : coeff f 0 ≠ 0) : (my_g f (my_b f prime_divisors_f)).coeff 0 = 1 := by

--   have hp : f.eval 0 ≠ 0 := by
...
theorem p_in_divisors_divides_ith_coeff_my_g (f : ℤ[X]) (i : ℕ) (p : prime_divisors f n) : p.val ∣ (my_g f (my_b f prime_divisors_f)).coeff i := by
  let b := my_b f prime_divisors_f
  rw [my_b]
  let ith_coeff_g := (my_g f b).coeff i
  have ith_coeff_g_eq : ith_coeff_g = b^i  / (f.eval 0) := by
    sorry
  have h_p_div_ith_coeff_g : p.val ∣ ith_coeff_g := by
    sorry
  done
...
--   cases hp with
--   | inl h =>
--     exact non_trivial_case hf (M:ℕ) h
--   | inr h =>
--     exact trivial_case M h
--   done
...
-- lemma trivial_case {f : ℤ[X]} (hf : f.natDegree ≠ 0) (M : ℤ) (hp : coeff f 0 = 0) : ∃ p, _root_.Prime p ∧ M ≤ p ∧  p ∣ f.eval p := by
--   have hp2 : f.eval 0 = 0 := by
--     rw[<-coeff_zero_eq_eval_zero]
--     exact hp
--     done
...
--   have hp3 (p : ℤ) : ∃ p, p ∣ f.eval p := by
--     use p
--     exact james_trivial hp
--     done

--   have hp4 : ∃ p, _root_.Prime p ∧ M ≤ p := by
--     sorry
--     done
--   apply?
...
theorem exists_prime_div_of_poly_eval (hp : (f : ℤ[X]) = X^2 + 3) (M:ℤ): ∃ p n, _root_.Prime p ∧ M ≤ p ∧ (p:ℤ) ∣ f.eval n := by
  apply two.one
  case hf =>
    rw [hp]
